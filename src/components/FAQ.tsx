...
const faqs = [
  ...
  },
  {
    ...
    a: "В системе 10 статусов: Не принят, Принят, Диагностика, Ожидание согласования, Ожидание запчастей, В ремонте, Готов, Выдан с ремонтом, Выдан без ремонта и Утиль. Каждый заказ проходит весь путь от приёмки до выдачи.",
  },
  {
    q: "Можно ли вести учёт запчастей и склада?",
    a: "Да. Есть отдельные модули «Запчасти» (каталог с ценами, подставляются в заказ) и «Склад» (остатки, поступления, списания в ремонт). Статус «Ожидание запчастей» помогает контролировать, какие ремонты ждут детали.",
  },
  {
    q: "Есть ли интеграция с телефонией и мессенджерами?",
    a: "Да. Модуль «Телефония» подключает IP-АТС — при звонке всплывает карточка клиента, ведётся история разговоров. Модуль «Мессенджеры» позволяет писать клиентам в WhatsApp и Telegram прямо из карточки заказа.",
  },
  {
    q: "Можно ли принимать оплату онлайн?",
    a: "Да. Модуль «Счёт онлайн» позволяет выставить клиенту счёт по ссылке — он оплачивает картой, СБП или другим способом, оплата сразу попадает в финансы. Модуль «Оплата» поддерживает наличные, эквайринг и СБП в кассе.",
  },
  {
    q: "Что входит в управление сайтом и видеонаблюдение?",
    a: "Модуль «Управление сайтом» позволяет управлять контентом сайта и принимать заявки прямо в CRM. «Видеонаблюдение» — подключение камер сервиса для мониторинга приёмки и ремонтного цеха в один клик.",
  },
  {
